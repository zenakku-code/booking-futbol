...
const DAYS_ARRAY = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"]
...
export default async function FieldDetailPage({
    ...
    // Cache the entire static content of the field and complex settings
    const data = await getOrSetCache(`field_detail_${id}`, async () => {
        const [field, inventory, complex] = await Promise.all([
            prisma.field.findUnique({
                where: { id },
                include: { complex: true }
            }),
            (prisma as any).inventoryItem.findMany({
                where: { field: { id } } // Assuming the relation exists or filter by complexId
            }).catch(() => []), // Graceful fail for inventory
            prisma.field.findUnique({ where: { id } }).then(f => 
                f ? (prisma as any).complex.findUnique({ 
                    where: { id: f.complexId },
                    select: { 
                        name: true, 
                        slug: true, 
                        address: true, 
        ...
        if (!field || complex?.slug !== slug) return null

        // Pre-generate available dates on server to reduce client-side JS workload (INP)
        const availableDates = []
        const today = new Date()
        const allowedDays = (field as any).availableDays ? (field as any).availableDays.split(',') : DAYS_ARRAY
        ...
        return { field, inventory, complex, availableDates }
    }, 300) // 5 minutes cache

    if (!data) notFound()
    const { field, inventory, complex, availableDates } = data

    const serverHasDeposit = Boolean(
        complex.downPaymentEnabled &&
        Number(complex.downPaymentFixed || 0) > 0 &&
        Number(complex.downPaymentFixed || 0) < Number(field.price)
    )
                ...
                <div className="grid lg:grid-cols-2 gap-8 lg:gap-20 items-start mt-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                                <div className="flex items-center gap-4 bg-white/5 backdrop-blur-3xl px-6 py-4 rounded-3xl border border-white/10 shadow-2xl group transition-all hover:bg-white/10">
                                    <div className="w-10 h-10 rounded-2xl bg-primary/10 flex items-center justify-center text-xl border border-primary/20">📍</div>
                                    <div className="flex flex-col">
                                        <span className="text-[9px] font-black uppercase tracking-[0.2em] text-gray-500 mb-0.5">Ubicación del Complejo</span>
                                        <span className="text-sm font-bold text-gray-200">{complex.address || 'Dirección no disponible'}</span>
                                    </div>
                                </div>

                                {complex.description && (
                                    <a 
                                        href={`https://wa.me/${complex.description.replace(/\D/g, '')}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center justify-center bg-[#25D366] hover:bg-[#128C7E] text-white w-14 h-14 rounded-full transition-all hover:scale-110 shadow-[0_0_30px_rgba(37,211,102,0.4)] active:scale-95 transition-transform"
                                    >
                    ...
                    <div className="mt-8 lg:mt-0 w-full overflow-hidden">
                        <BookingFlow
                            ...
                            paymentSettings={{
                                downPaymentEnabled: complex.downPaymentEnabled || false,
                                downPaymentFixed: Number(complex.downPaymentFixed || 0)
                            }}
